...
void decimalparabinario(double numero, char bin[maxtam], int *tamnumero){
    ...
    printf("O número %lf em binário é: ",numero);
...
void expozero(double numero, char bin[maxtam],char decimal[maxtam], int mantissa, int *tamnumero){
	if(mantissa>*tamnumero)
		mantissa = *tamnumero;
	int inicio = comeco(bin);
...
int main(){
    ...
    do
    {
		if(ops==1){
			...
			scanf("%s",&aux2);
			printf("Digite o maior valor de expoente. (U) \n");
			scanf("%s",&aux3);
		}
		...
		if( test2 > test3 || test1<=0 || vv !=0 || aa !=0 || bb !=0 ){
			...
			printf("Digite a precisão da mantissa. (M) \n");
			scanf("%s",&aux1);
			printf("Digite o menor valor de expoente. (L) \n");
			scanf("%s",&aux2);
			printf("Digite o maior valor de expoente. (U) \n");
		...
		printf("Digite um número decimal a ser convertido.\n");
    	scanf("%s", &NUMERO);
        ...
        printf("\nConfiguração atual da máquina: (Num: %lf | M :%d | L: %d | U: %d)\n", numero,mantissa,menorE,maiorE);
        ...
        case 1:
        	...
        	printf("Digite um número em binário a ser convertido. \n");
        	scanf("%s",&numero2); tam3= strlen(numero2); numero2[tam3] = '\0';
        	printf("O número %s em decimal é: ",numero2);
            binarioparadecimal(numero2,numero);
            printf("\n");
            break;
        case 2:
        	printf("Configuração atual da máquina: (Num: %lf | M :%d | L: %d | U: %d)\n", numero,mantissa,menorE,maiorE);
        ...
        case 3://Sair do programa
			...
			if(numero<0){
				...
				if(numero>=(-0.99999999999999999999999) && numero <= 0.99999999999999999999999 ){
				printf("Número em sinal e amplitude: ");
            	normalizarNega(numero,normalizado,bin,mantissa,&contador,&tamnumero);
            	printf("\n");
            }else{
			...
			printf("Número em complemento de 1:");
